...
function UA (uaString){
    ...
    if (!uaString) {
        ...
        if (normalized = uaString.match(/^(\w+)\/(\d+)(?:\.(\d+)(?:\.(\d+))?)?$/i)) {
            this.ua = {
		...
		uaString = uaString.replace(/ (Snapchat)\/(\d+)\.(\d+)\.(\d+)\.(\d+)/i, "");
		...
		uaString = uaString.replace(/ (Crosswalk)\/(\d+)\.(\d+)\.(\d+)\.(\d+)/i, "");
		...
		if (
            ...
            (this.ua.family === "android" && Number(this.ua.major + '.' + this.ua.minor) >= 4.3) || 
